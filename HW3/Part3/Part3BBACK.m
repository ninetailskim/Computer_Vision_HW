            ...
            while (flag)
                ...
                for si = -spacew:spacew
                    for sj = -spacew:spacew
                        if(i + si > 0 && i + si <= w && j + sj >0 && j + sj <= h)
                            filter(i + si,j + sj) = 1;
                        end
                    end
                end
...
imshow(res);
% figure;
% SCA = [reshape(B(:,:,1),[1 width * height]);reshape(B(:,:,2),[1 width * height]);reshape(B(:,:,3),[1 width * height])];
% % c = 
% scatter3(SCA(1,:),SCA(2,:),SCA(3,:),'.',reshape(B,[1 width * height 3]));
% figure;
% scatter3(reshape(res(:,:,1),[1 width * height]),reshape(res(:,:,2),[1 width * height]),reshape(res(:,:,3),[1 width * height]));